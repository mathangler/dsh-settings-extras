...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Follow-up schedule for one stale answer.
     *
     * The rescan behind a stale answer is not a fixed cost: the first panel open
     * after a boot measured 27.5s on the machine this was built against, because
     * that pass re-reads the live session's whole log. A retry that arrives while
     * the scan is still running joins it, so every follow-up is one small request
     * that answers from the cache — the schedule starts fast (0.7s) for the usual
     * brief rescan and eases off to one every 2.5s, covering about 28s in twelve
     * requests. The count is bounded, so a host that never finishes rescanning
     * cannot turn this into a poll; the explicit refresh button remains.
     */
    const STALE_RETRY_MS = 700;

    /** Ceiling for the wait between follow-ups. */
    const STALE_RETRY_MAX_MS = 2500;

    /** How many follow-ups one stale answer may queue. */
     ...
     */
    function staleRetryDelay(tries) {
          ...
          return hostCall(force ? 'refresh' : 'summary', {}).then((result) => {
            ...
            if (result && Array.isArray(result.days)) {
              ...
              if (staleFollowUp(result, tries)) {
                setRefreshing(true);
                timer.current = setTimeout(() => load(false, tries + 1), staleRetryDelay(tries));
                return;
              }
              setRefreshing(false);
              return;
